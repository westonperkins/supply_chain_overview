...
**Body classification tables below (n counts, per-section listings) reflect the state at Pass P end. Pass Q re-authored 3 of the 29 power-cluster edges under §4 and left 10 as reviewed-and-undeterminable; Pass Q.1 REVERTED one of those three (see §1 in the Pass Q.1 update section). The `## Pass Q update` + `## Pass Q.1 update` sections immediately below the Summary are authoritative for post-Q.1 status; the historical body is preserved unchanged for continuity and audit-trail readability (this doc is not auto-regenerated by a committed script).**

## Summary

| classification | count | note |
|---|---|---|
| unclassified | 119 | pre-Q; unchanged by Q.1 |
| supply_share (mining/refining) | 40 | pre-Q; unchanged by Q.1 |
| dependency (K.1 §4.3 re-author) | 36 | pre-Q; unchanged by Q.1 |
| dependency (Pass Q §4 re-author) | **2** | Pass Q authored 3; Pass Q.1 reverted `gev→nextera` to undeterminable |
| dependency (Pass Q, reviewed and undeterminable) | **11** | Pass Q left 10; +1 from the Q.1 REVERT of `gev→nextera` |
| supply_share_basis | 35 | pre-Q; unchanged by Q.1 |
| suspect_cost_or_volume | 27 | pre-Q; unchanged by Q.1 |
| cost_or_mass_basis | 2 | pre-Q; unchanged by Q.1 |
| **total edges** | 259 | |
...
## Pass Q.1 update (2026-08-18)

Pass Q.1 is a correction pass. It revises the Pass Q table above and adds durable per-edge notes to the 10-plus-one edges reviewed and undeterminable. No new §4-basis re-authors are added; the value that changed goes DOWN, not up.

**Revised Pass Q table (post-Q.1):**

| edge | Pass Q after | Pass Q.1 after | status | reason |
|---|---:|---:|---|---|
| `company:ge_vernova → company:nextera_energy` | **0.30** | **0.25** | REVERTED to undeterminable | Pass Q's source_note stated "no single sourced document reference"; the reasoning cited Hitachi Energy as a third-supplier which is not a graph node; the value landed the NextEra `power_equipment` bucket sum on exactly 0.80 (the completeness-test threshold), and Pass Q edited the pinned shortfall file in the same commit. All three concerns pointed to the same fix. See Pass Q.1 §1 in `docs/generated/replay/grading.md`. |
| `company:quanta_services → company:nextera_energy` | 0.30 | 0.30 | retained; confidence `estimate` → `inference` | The source_note itself said "Confidence: inference." Pass Q.1 §5 aligns the flag with the note. |
| `company:vertiv → facility:the_citadel` | 0.35 | 0.35 | retained; confidence `estimate` → `inference` | The source_note itself said "the specific within-class share at The Citadel is inference." Pass Q.1 §5 aligns the flag with the note. |

**11 edges now sit in `dependency (Pass Q, reviewed and undeterminable)`** — the original 10 plus the reverted `gev→nextera`. All 11 carry a `[Pass Q §4: examined, undeterminable — <reason>]` prefix in their `source_note` so a future author sees the review immediately and the audit doesn't re-flag them as unexamined. The reasons live in the edges themselves (Q.1 §3), not only in the report.

**Downstream effects of the REVERT:**

- NextEra `power_equipment` bucket sum: **0.80 → 0.75**. Reverts to the pre-Q pinned-shortfall state.
- NextEra `inbound_hhi`: 0.6080000000000001 → **0.58** (exact match to pre-Q).
- GE Vernova `outbound_criticality`: 0.19473684861108456 → **0.1819268342116367** (exact match to pre-Q).
- Small `outbound` deltas on cascade-reached country/mineral/product nodes (copper, gallium, rf_power_semis, china, usa, chile, drc, peru, canada, japan, south_korea) reverse to pre-Q values.
- Zero severity movement on any node (all touched-by-cascade nodes are either unscored or inbound-dominant with unchanged inbound).
- `nextera_energy/supplies` is restored to `backend/tests/pinned/known_bucket_shortfalls.txt`.

**Interaction findings recorded during Pass Q.1:**

- The Pass Q caveat check (A/B/C branches) passed a Quanta Services caveat whose prose asserted `Inbound HHI reads 1.0` — false under Pass N's noisy-OR aggregator (current value: 0.30). See Pass Q.1 §2. Fixed. A fourth branch (**D — false-assertion**) is now on the record for future caveat audits.
- A guard test `test_modeling_caveat_numbers_are_current` mechanically runs the branch-D check on every committed caveat every pass; the defect being caught is a caveat asserting a number that no longer matches the node's computed value.
- `config/scoring.yaml`'s concentration TODO block carried the same stale claim ("Quanta = 1.0", "xAI/OpenAI = 0.78", copper "~0.25"). All three rewritten to state the structural fact without a specific numeral. No key value changed; comment-only edits.
